...
const props = defineProps<{ disabled: boolean; udpAllocated: boolean }>()
...
<template>
  <form class="port-add" @submit.prevent="submit">
    <fieldset class="port-fields" :disabled="disabled">
      ...
      <button type="submit">追加预留号码</button>
    </fieldset>
    <p v-if="error" class="fault" role="alert">{{ error }}</p>
    <p class="port-help">从配置的端口池追加预留{{ udpAllocated ? ' TCP 和 UDP' : ' TCP' }} 端口，之后可按需启用映射。</p>
  </form>
</template>
