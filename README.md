...
无参数且在终端中运行时提供持续运行的五项菜单。失败后显示结果并等待回车，再返回菜单。静态页面（主菜单、参数输入、确认页、结果页、日志页）按 `Ctrl+C` 直接退出脚本；安装、更新、恢复备份和仅卸载 Agent 执行中按 `Ctrl+C` 软中断并回退，全部卸载开始后不能用 `Ctrl+C` 取消。主菜单 `0` 退出。结果页可用 `L` 查看诊断日志；适合安全重试时提供 `R`，安装/更新还可用 `E` 修改版本。非交互调用失败仍返回非零退出码。完整说明和流程图见[安装脚本交互流程](docs/development/installer-workflow.md)。

主菜单 **2. 查看状态** 内提供 Incus 和 Agent 各自的启动、停止操作。操作后核对服务状态，回车返回状态子菜单；`R` 刷新，`0` 或直接回车返回主菜单。启停执行中按 `Ctrl+C` 会等待服务操作完成并核对结果，再返回状态子菜单。
