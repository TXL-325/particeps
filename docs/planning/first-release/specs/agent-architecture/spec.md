...
原单 Change 的历史实施顺序如下；新的交付顺序见交付计划：
...
原拆分判断基于共用核心接口选择单 Change。当前基础代码已形成，拟通过顺序小交付与明确契约继续实施；不因此取消共同的恢复、集成与原 81 项全量验收。
...
全部有效验收均有真实结果，经 Comet 独立复核与 Verify 后再进入交付收尾。本文件保留完整首期目标，不作为当前局部交付已完成的证明。
