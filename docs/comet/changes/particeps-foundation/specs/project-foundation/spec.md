...
本轮菜单优化保留安装/升级、状态、回滚、全部卸载、只卸 Agent 五项入口，并持续返回发起操作的菜单；主菜单 0 退出，无效输入重新选择。操作失败先停止后续步骤，结果页显示失败阶段、原因、实际变更和日志位置，等待一次回车再返回。执行中 Ctrl+C 取消本次任务，完成必要临时资源清理并显示取消结果后回菜单，不隐式撤销已完成的变更。提供执行前摘要、参数校验与会话内保留、阶段进度和真实服务状态验证。失败页支持查看诊断日志，并仅在适合安全重试时重新预检后执行；非交互失败退出非零且不等待输入。日志不保存初始管理员密码。对应本次 A13，PTY 与 mock 只证明脚本流程，不代替实机安装生命周期。
