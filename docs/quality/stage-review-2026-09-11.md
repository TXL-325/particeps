...
本报告保留阶段审阅时的代码、Git 和环境快照。同日后续已配置 VMnet2 多 IPv4、两个小鸡 IPv6 网段及现有小鸡的静态 IPv6，实际环境变化见 [本地网络实验环境](../development/network-lab.md)；这些手工配置不代表报告中尚缺少的应用网络功能已实现。

2026-09-13 当前 foundation 已有 13 项局部门槛，用户选择按 58bab0a 同步高优先部署项 A13；最新确认、发布及母机证据见[Native 进度同步](native-progress-sync-2026-09-12.md)和[交付计划](../planning/delivery-plan.md)。下文 81 项编号、状态版本 20、Build 阶段及当时缺口均属于历史快照，当前阶段和确认边界读取 Runtime。

后续已实现并实测 F03 的 IPv4 端口功能，同时更新了实验母机 Agent；最新代码地图、实际验证及仍未实现的部分见 [F03 批次记录](f03-port-forwarding-2026-09-11.md)。下文的旧服务、空转发和代码缺口描述属于本次阶段审阅的历史快照。
